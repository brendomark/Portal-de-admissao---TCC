...
$EXTENSAO = pathinfo($_FILES["imagem"]["name"], PATHINFO_EXTENSION);
    if(in_array($EXTENSAO,$FORMATOS)):
        ...
        if(move_uploaded_file($TEMP,$PASTA.$novoNome)):
            $sql = "SELECT FOTO_PERFIL FROM PFUNC FUN WHERE FUN.CPF ='{$CPF}'";
            $dados = mysqli_query($conexao,$sql);
            $local_foto =mysqli_fetch_assoc($dados);
            unlink($local_foto["FOTO_PERFIL"]);

            $sql = "UPDATE pfunc set FOTO_PERFIL='$nomeBanco' WHERE CPF = '{$CPF}' ";
    ...
    $PASTA_DOC = "docs_clientes/";
        ...
        if(in_array($CPF_PDF,$FORMATOS_DOC)):
            ...
            if(move_uploaded_file($TEMP_CPFRG,$PASTA_DOC.$cpfnome)):
                $sql_cpf = "UPDATE pfunc SET ANEXO_CPF = '../$cpfbanco' WHERE CPF = '$CPF'";
        ...
        if(in_array($RG_PDF,$FORMATOS_DOC)):
            ...
            if(move_uploaded_file($TEMP_CPFRG,$PASTA_DOC.$rgnome)):
                $sql_rg = "UPDATE pfunc SET ANEXO_RG = '../$rgbanco' WHERE CPF =  '$CPF'";
        ...
        if(in_array($TITULO_PDF,$FORMATOS_DOC)):
            ...
            if(move_uploaded_file($TEMP_TITULO,$PASTA_DOC.$titulonome)):
                $sql_titulo = "UPDATE pfunc SET ANEXO_TITULO = '../$titulobanco' WHERE CPF =  '$CPF'";
        ...
        if(in_array($CTPS_PDF,$FORMATOS_DOC)):
            ...
            if(move_uploaded_file($TEMP_CTPS,$PASTA_DOC.$ctpsnome)):
                $sql_ctps = "UPDATE pfunc SET ANEXO_CTPS = '../$ctpsbanco' WHERE CPF =  '$CPF'";
        ...
        if(in_array($MILITAR_PDF,$FORMATOS_DOC)):
            ...
            if(move_uploaded_file($TEMP_MILITAR,$PASTA_DOC.$militarnome)):
                $sql_militar = "UPDATE pfunc SET ANEXO_MILITAR = '../$militarbanco' WHERE CPF =  '$CPF'";
        ...
        if(in_array($END_PDF,$FORMATOS_DOC)):
            ...
            if(move_uploaded_file($TEMP_END,$PASTA_DOC.$endnome)):
                $sql_militar = "UPDATE pfunc SET ANEXO_END = '../$endbanco' WHERE CPF =  '$CPF'";
                mysqli_query($conexao,$sql_militar);
                $_SESSION['mensagem'] = "ENVIADO!";
            endif;
        endif;
...
$sqlpes = "UPDATE PPESSOA SET CODSECAO = '$SECAO', PRIMEIROEMP = '$PRIMEIRO_EMP', DTADMISSAO =STR_TO_DATE('$DT_ADMISSAO', '%d/%m/%Y'), GESTOR ='$GESTOR', CODFUNCAO = '$FUNCAO', TPSALARIO = '$TP_SALARIO', SALARIO = '$SALARIO', JORNADA = '$JORNADA', HORARIOS = '$HORARIOS', MATRICULA = '$MATRICULA', LOCALSERVICO = '$LOCAL_SERVE', TPJORNADA = '$TP_JORNADA', CONTRATACAO = '$CONTRATACAO', PRAZO = '$PRAZO' WHERE CPF = '{$CPF}'";
